 ...
 * Author Contact: marcel6301 (Discord)
 * InitialScript by RedAlert
 */
...
async function initVillageSnipe(groupId) {
    ...
    setTimeout(function () {
        // set the default destination village (robust desktop + mobile)
        const extracted = extractDestinationVillageCoords();
        const destinationVillage = extracted.coords;

        // Always log which variant was used
        console.info(
            `${scriptInfo()} raDestinationVillage extraction variant: ${extracted.variant}`,
            { coords: extracted.coords, raw: extracted.raw }
        );
...
function prepareContent(groups, unitsTable) {
    ...
    <div class="ra-mb15" id="raUnitPresets">
        <a href="javascript:void(0);" class="btn" id="presetEmpty">Leer</a>
        <a href="javascript:void(0);" class="btn" id="presetDeff">Deff</a>
        <a href="javascript:void(0);" class="btn" id="presetOff">Off</a>
    </div>
		<div class="ra-mb15">
...
// Unit Preset Buttons
// ===============================
function bindUnitPresetButtons() {

    function setUnits(units) {
        // alle abwählen
        jQuery('.ra-unit-selector').prop('checked', false);

        // gewünschte aktivieren
        units.forEach(unit => {
            jQuery(`#unit_${unit}`).prop('checked', true);
        });

        // speichern wie Originalscript
        const chosenUnits = [];
    ...
    // ===== Leer =====
    jQuery('#presetEmpty').off().on('click', function () {
        setUnits([]);
    });

    // ===== Deff =====
    jQuery('#presetDeff').off().on('click', function () {
        const deff = ['spear','sword','heavy'];

        // Archer nur wenn Welt sie hat
    ...
    // ===== Off =====
    jQuery('#presetOff').off().on('click', function () {
        setUnits(['axe','light','ram']);
    });
}
...
function extractDestinationVillageCoords() {
    try {
        // MOBILE
        if (mobiledevice) {
            let raw = '';
            let coords = '';
            let variant = 'mobile-failed';

            // 1) Preferred: exact mobile key/value row where first span is "Koordinaten:"
            const coordRow = jQuery('.mobileKeyValue > div')
                .filter(function () {
                    return (
                        jQuery(this).find('span:first').text().trim() ===
                        'Koordinaten:'
                    );
                })
                .first();
            ...
            if (coordRow.length) {
                raw = coordRow.find('span:eq(1)').text().trim();
                const m = raw.match(/\d+\|\d+/);
                if (m) {
                    coords = m[0];
                    variant = 'mobile-label';
                    return { coords, variant, raw };
                }
            }

            // 2) Fallback: scan only within the .mobileKeyValue container for the first coords pattern
            raw = jQuery('.mobileKeyValue').first().text();
            ...
            if (m2) {
                ...
                return {
                    coords,
                    variant,
                    raw: '(scanned .mobileKeyValue text)',
                };
            }
        ...
        // DESKTOP
        // 1) Preferred: locate row where first td is exactly "Koordinaten:" and read second td
        const labelRow = jQuery('#content_value table.vis tr')
            .filter(function () {
                return (
                    jQuery(this).find('td:first').text().trim() ===
                    'Koordinaten:'
                );
            })
            .first();
        ...
        if (labelRow && labelRow.length) {
            const raw = labelRow.find('td:eq(1)').text().trim();
            const match = raw.match(/\d+\|\d+/);
            if (match) {
                return { coords: match[0], variant: 'desktop-label', raw };
            }
        }

        // 2) Fallback: scan rows inside the main village info table for first coords pattern
        let fallbackRaw = '';
        let found = '';
        jQuery('#content_value table.vis tr').each(function () {
            const txt = jQuery(this).text();
            const m = txt.match(/\d+\|\d+/);
            if (m && m[0] && !found) {
                found = m[0];
                fallbackRaw = txt.trim();
            }
        });
        ...
        // 3) Fail
        return { coords: '', variant: 'desktop-failed', raw: '' };
    } catch (e) {
        return { coords: '', variant: 'exception', raw: String(e) };
    }
}
